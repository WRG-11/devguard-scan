...
# The Python parity step needs the canonical detection source. Clone the public
# WRG-11/wrg-devguard repo and pass -WrgDevguardSrc <checkout>/src (or set
...
if (-not $WrgDevguardSrc -or -not (Test-Path $WrgDevguardSrc)) {
    ...
    Write-Host "  Provide -WrgDevguardSrc <checkout>/src (clone WRG-11/wrg-devguard) to run parity." -ForegroundColor Yellow
} else {
